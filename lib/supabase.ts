...
function getConfig() {
  ...
  if (!url || !key) {
    throw new SupabaseConfigurationError("Supabase no estÃ¡ configurado")
  }
...
export async function insertOpinion(type: OpinionType, message: string) {
  ...
  if (!response.ok) {
    ...
    throw new Error("No se pudo almacenar la opiniÃ³n")
  }
}
...
export async function getOpinionById(id: string): Promise<Opinion | null> {
  ...
  if (!response.ok) {
    const detail = await response.text()
    console.error("Supabase opinion lookup failed", response.status, detail.slice(0, 300))
